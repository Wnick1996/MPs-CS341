...
int starttimer()
{
  ...
  subtract = inpt(TIMER0_COUNT_PORT);	/* read in LSB of count */
  subtract |= (inpt(TIMER0_COUNT_PORT)<<8); /* then MSB of count */
  startcount = COUNTS_PER_TICK * tickcount + (COUNTS_PER_TICK - subtract);
  timer_running = TRUE;
  return T_OK;
}
...
int stoptimer( int *interval )
{
  ...
  subtract = inpt(TIMER0_COUNT_PORT);	/* read in LSB of count */
  subtract |= (inpt(TIMER0_COUNT_PORT)<<8); /* then MSB of count */
  int dc = COUNTS_PER_TICK * tickcount + (COUNTS_PER_TICK - subtract);
  *interval = (dc - startcount)*(100000.0/COUNTS_PER_SEC);
#ifdef DEBUG
